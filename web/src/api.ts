...
export type Row = { id: string; number: string; valid: boolean; invalid_reason: string | null; status: string; eta: string | null; next_milestone: string | null; severity: number; band: string; exceptions: Finding[]; events?: Record<string, string>[]; explanation?: { source?: string; text: string; action?: string; draft_message: string }; origin?: string; destination?: string }
